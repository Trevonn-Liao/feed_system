...
type AuthService struct {
	...
	tokens    *jwt.Manager
	bloom     interface{ MightContain(string) bool }
	snowflake *snowflake.Generator
	guard     *requestguard.Guard
}

func NewAuthService(users repository.UserRepository, tokens *jwt.Manager, bloom interface{ MightContain(string) bool }, snowflake *snowflake.Generator, guard *requestguard.Guard) *AuthService {
	return &AuthService{
		users:     users,
		tokens:    tokens,
		bloom:     bloom,
